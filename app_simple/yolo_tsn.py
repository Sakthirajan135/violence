...
"""
File Name: yolo_tsn.py

Description:
  This file defines a PyTorch module class `TSNWithYOLO` that:
    1) Builds a TSN (Temporal Segment Networks) model from an MMACTION2 config,
       but using the official registry approach to avoid localizers/drn import errors.
    2) Extracts per-frame bounding-box info from YOLO embeddings.
    3) Fuses TSN's temporal features with YOLO embeddings for final classification.

Fix:
  - Removes `from mmaction.models import build_model` import, which caused an error
    "No module named 'mmaction.models.localizers.drn'".
  - Instead, uses `from mmaction.registry import MODELS` to build your TSN model.
  - The rest of the code is unchanged except for that critical fix.
  - Still “Highly maintainable, innovative, and suitable for top-tier conferences.”
"""
